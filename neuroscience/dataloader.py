...
class DataLoaderFinetune(DataLoader):
...
class DataLoaderMultiviewSubstructContext(torch.utils.data.DataLoader):
    r"""Data loader which merges data objects from a
    :class:`torch_geometric.data.dataset` to a mini-batch.
    Args:
        dataset (Dataset): The dataset from which to load the data.
    ...
    def __init__(self, dataset, batch_size=1, shuffle=True, **kwargs):
        super(DataLoaderMultiviewSubstructContext, self).__init__(
            dataset,
            batch_size,
            shuffle,
            collate_fn=lambda data_list: BatchSubstructContext.from_data_list_list(data_list),
            **kwargs)
